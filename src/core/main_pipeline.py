...
# 로컬 모듈 임포트
from processing.chunking import DocumentProcessor, Chunk
from processing.text_normalization import TextNormalizer
...
if USE_LOCAL:
    # 로컬 파일 저장소 사용
    from storage.local_storage import LocalVectorStore as VectorDatabase
    print("✅ 로컬 파일 저장소 모드")
else:
    # PostgreSQL 데이터베이스 사용
    from storage.vector_db import VectorDatabase, DatabaseConfig, LabDocument
...
def main():
    ...
    if USE_LOCAL:
        ...
        orchestrator = CrawlOrchestrator(
            ...
            device='cpu',
            local_data_dir='./data/crawl_data'
        )
    else:
        # PostgreSQL 사용 (주석처리 - 나중에 복원 가능)
